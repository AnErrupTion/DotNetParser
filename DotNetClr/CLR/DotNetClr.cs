...
public partial class DotNetClr
{
    ...
    /// Loaded DLLS
    /// </summary>
    private Dictionary<string, DotNetFile> dlls = new();
    /// <summary>
    /// Callstack
    /// </summary>
    private List<CallStackItem> CallStack = new();
    /// <summary>
    /// List of custom internal methods
    /// </summary>
    private Dictionary<string, ClrCustomInternalMethod> CustomInternalMethods = new();
    /// <summary>
    ...
    public void Start(string[] args = null)
    {
        ...
        if (args != null)
        {
            var itms = new MethodArgStack[args.Length];
    ...
    {
        if (InitCorLib)
            ResolveDLL("System.Private.CoreLib"); //Always resolve mscorlib, in case the exe uses .net core
        ...
        foreach (var t in file.Types)
        {
            foreach (var m in t.Methods)
            {
                if (m.Name == ".cctor" && m.IsStatic)
                {
                    if (ShouldPrintDebugMessages)
                    {
                        NativeHost.Console_WriteLine("Creating " + t.FullName + "." + m.Name);
                    }
    ...
    {
        var stack = new CustomList<MethodArgStack>(m.MaxStackSize);
        if (!Running)
            return null;
        ...
        if (m.IsInternalCall | m.IsImplementedByRuntime)
        {
            ...
            foreach (var item in CustomInternalMethods)
            {
                if (item.Key == properName)
                {
                    ...
                    //Don't forget to remove item params
                    ...
                    {
                        var StartParmIndex = -1;
                        var EndParmIndex = -1;
                        var paramsRead = 0;
                        var foundEndIndex = false;
                        if (m.AmountOfParms > 0)
                        {
                            var endParam = m.SignatureInfo.Params[^1];
                            for (var i4 = stack.Count - 1; i4 >= 0; i4--)
                            {
        ...
        #endregion
        var Localstack = new MethodArgStack[256];
        if (addToCallStack)
        {
        ...
        for (i = 0; i < code.Length; i++)
        {
            ...
            switch ((OpCodes.OpCodesList)item.OpCode)
            {
                ...
                case OpCodes.OpCodesList.Br_S:
                {
                    ...
                    var i2 = item.Position + (int)item.Operand + 1;
                    var inst = decompiler.GetInstructionAtOffset(i2, -1);
                ...
                case OpCodes.OpCodesList.Brfalse_S:
                {
                    if (stack.Count == 0)
                    {
                        clrError("Stack is empty. Instruction: brfalse.s", "Internal");
                        return null;
                    }
                    var s = stack[^1];
                    stack.RemoveAt(stack.Count - 1);
                ...
                {
                    if (stack[^1].value == null)
                    {
                        stack.RemoveAt(stack.Count - 1);
                        continue;
                    }
                    var exec = false;
                    if (stack[^1].type != StackItemType.Int32)
                    {
                        if (stack[^1].type != StackItemType.ldnull)
                        {
                            exec = true;
                        }
                    }
                ...
                case OpCodes.OpCodesList.Ldfld:
                {
                    ...
                    var info = item.Operand as FieldInfo;
                    DotNetField f2 = null;
                ...
                case OpCodes.OpCodesList.Ldftn:
                {
                    ...
                    var obj2 = stack[^1]; //the compiler generated object to call the method on
                ...
                case OpCodes.OpCodesList.Stfld:
                {
                    ...
                    if (data.Fields.ContainsKey(f2.Name))
                    {
                        Objects.ObjectRefs[(int)obj.value].Fields[f2.Name] = stack[^1];
                    }
                    else
                    {
                        Objects.ObjectRefs[(int)obj.value].Fields.Add(f2.Name, stack[^1]);
                    }
                    stack.RemoveAt(stack.Count - 1);
                    stack.RemoveAt(stack.Count - 1);
                    break;
                ...
                case OpCodes.OpCodesList.Stsfld:
                {
                    ...
                    {
                        //create field
                        StaticFieldHolder.staticFields.Add(new StaticField() { theField = f2, value = stack[^1] });
                    }
                    if (f2 == null)
    ...
    public MethodArgStack CreateObject(DotNetMethod Constructor, CustomList<MethodArgStack> Constructorparamss)
    {
        ...
        var a = new MethodArgStack() { ObjectContructor = Constructor, ObjectType = Constructor.Parent, type = StackItemType.Object, value = objAddr };
        ...
        {
            NativeHost.Console_WriteLine("Error occured, returning null.");
    ...
    private void BranchWithOp(ILInstruction instruction, CustomList<MethodArgStack> stack, IlDecompiler decompiler, MathOperations.Operation op, ref int i)
    {
        ...
        {
            var i2 = instruction.Position + (int)instruction.Operand + 1;
            var inst = decompiler.GetInstructionAtOffset(i2, -1);
    ...
    {
        var old = NativeHost.Console_ForegroundColor;
        NativeHost.Console_ForegroundColor = c;
        NativeHost.Console_WriteLine(s);
        NativeHost.Console_ForegroundColor = old;
    }
    ...
    internal bool InternalCallMethod(InlineMethodOperandData call, DotNetMethod m, bool addToCallStack, bool IsVirt, bool isConstructor, CustomList<MethodArgStack> stack, MethodArgStack constructorObj = null)
    {
        ...
        if (call.RVA != 0)
        {
            ...
            if (m2 == null)
            {
                NativeHost.Console_WriteLine($"Cannot resolve called method: {call.NameSpace}.{call.ClassName}.{call.FunctionName}(). Function signature is {call.Signature}");
                return false;
            }
        }
        else
        ...
        //Extract the params
        var StartParmIndex = stack.Count - m2.AmountOfParms;
        var EndParmIndex = stack.Count - 1;
        ...
        if (StartParmIndex != -1)
        {
            for (var i5 = StartParmIndex; i5 < EndParmIndex + 1; i5++)
            {
                var itm5 = stack[i5];
                newParms.Add(itm5);
            }
        ...
        returnValue = RunMethod(m2, m != null ? m.Parent.File : m2.Parent.File, newParms, addToCallStack);
    ...
    {
        return m.Parent.FullName switch
        {
            "System.String" when obj.type == StackItemType.String => true,
            "System.IntPtr" when obj.type == StackItemType.IntPtr => true,
            "System.Byte" when obj.type == StackItemType.Int32 => true,
            "System.SByte" when obj.type == StackItemType.Int32 => true,
            "System.UInt16" when obj.type == StackItemType.Int32 => true,
            "System.Int16" when obj.type == StackItemType.Int32 => true,
            "System.UInt32" when obj.type == StackItemType.Int32 => true,
            "System.Int32" when obj.type == StackItemType.Int32 => true,
            "System.UInt64" when obj.type == StackItemType.Int64 => true,
            "System.Int64" when obj.type == StackItemType.Int64 => true,
            "System.Char" when obj.type == StackItemType.Int32 => true,
            "System.Object" when obj.type == StackItemType.Object => true,
            "System.Type" when obj.type == StackItemType.Object => true,
            "System.Boolean" when obj.type == StackItemType.Int32 => true,
            "System.Decimal" when obj.type == StackItemType.Object => true,
            "System.Decimal" when obj.type == StackItemType.Decimal => true,
            _ => false
        };
    }
    #endregion
...
{
    public static readonly List<ArrayRef> ArrayRefs = new();
    ...
    public static ArrayRef AllocArray(int arrayLen)
    {
        ...
        {
            Length = arrayLen,
            Items = new MethodArgStack[arrayLen]
        };
...
{
    public static readonly List<ObjectRef> ObjectRefs = new();
    private static int CurrentIndex;
    public static int GetIndexFromRef(MethodArgStack r)
    {
        return (int)r.value;
...
{
    public readonly Dictionary<string, MethodArgStack> Fields = new();
