...
public class USStreamReader
{
    ...
    public USStream Read()
    {
        ...
        var eightByteStart = false;
        for (var i = 1; i < _dataSize; i++)
        {
            ...
            var end = false;
            for (var i2 = 0; i2 < ActualStringLen / 2; i2++)
            {
                try
                {
            ...
            var s = Encoding.Unicode.GetString(bytes.ToArray());
    ...
    {
        ReadInt32Bit7(r, out var result);
        return result;
    }
