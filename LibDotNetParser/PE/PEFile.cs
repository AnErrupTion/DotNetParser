...
public class PEFile
{
    ...
    {
        var fs = NativeHost.File_ReadAllBytes(FilePath);
        Init(fs);
    }
    ...
    private void Init(byte[] data)
    {
        ...
        {
            NativeHost.Console_WriteLine("No clr header!");
        ...
        var pos = (long)BinUtil.RVAToOffset(ClrHeader.MetaDataDirectoryAddress, PeHeader.Sections);
        ...
        ClrMetaDataHeader = new MetadataHeader
        {
            ...
            VersionStringLength = r.ReadUInt32()
        };
        ...
        for (var i = 0; i < ClrMetaDataHeader.NumberOfStreams; i++)
        {
            var hdr = new StreamHeader
            {
                Offset = r.ReadUInt32(),
                Size = r.ReadUInt32(),
                Name = r.ReadNullTermFourByteAlignedString()
            };
        ...
        //Parse the #~ stream
        var TableStreamR = new BinaryReader(new MemoryStream(GetStreamBytes("#~", r)));
    ...
    private List<Section> ReadSectionsList(int numberOfSections, BinaryReader _assemblyReader)
    {
        ...
        for (var i = 0; i < numberOfSections; i++)
        {
            var s = new Section
            {
                Name = _assemblyReader.ReadNullTermString(8),
