...
public class IlDecompiler
{
    ...
    {
        var inr = new List<ILInstruction>();
        var Instructions = 0;
    ...
    public ILInstruction GetInstructionAtOffset(int Offset, int relPostion)
    {
        ...
        if (opCode == null)
        {
            NativeHost.Console_WriteLine("ILDecompiler failed decompilation.");
            opCode = OpCodes.SingleOpCodes[0];
        }

        var ret = new ILInstruction() { OpCode = opCode.Value, OpCodeName = opCode.Name, OperandType = opCode.OpCodeOperandType, Position = Offset, RelPosition = relPostion, Size = size };
        ...
        switch (opCode.OpCodeOperandType)
        {
            ...
            {
                var fi = code[Offset + 1];
                var s2 = code[Offset + 2];
                var t = code[Offset + 3];
                var Tabel = code[Offset + 4];
                var num2 = new byte[] { fi, s2, t, 0 };
            ...
            case OpCodeOperandType.InlineField:
            {
                ...
                else if (f == 10)
                {
                    ...
                    DecodeMemberRefParent(c.Class, out var type, out var row2);
            ...
            case OpCodeOperandType.InlineMethod:
            {
                ...
                var s2 = code[Offset + 2];
                var t = code[Offset + 3];
                var f = code[Offset + 4]; //Method type. 6=Method,10=MemberRef
                var num2 = new byte[] { fi, s2, t, 0 };
                ...
                if (f == 10) //MemberRef
                {
                    ...
                    DecodeMemberRefParent(c.Class, out var tabel, out var row);
                    ...
                    else
                    {
                        ...
                        {
                            // NativeHost.Console_WriteLine($"[ILDecompiler: WARN] Cannot resolve method RVA. Are you missing a call to AddReference()??. Method data: {anamespace}.{typeName}.{funcName}");
                        }
                    }
                ...
                else if (f == 6)//method
                {
                    ...
                    var name = mainFile.Backend.ClrStringsStream.GetByOffset(c.Name);
                    //Now, resolve this method
                    DotNetMethod m = null;
                    ...
                    var className = "CannotFind";
                    var Namespace = "CannotFind";
                ...
                else if (f == 0x2B)
                {
                    ...
                    else if (fa == 1)
                    {
                        ...
                        #region Decode
                        //Decode the class bytes
                        DecodeMemberRefParent(c.Class, out var tabel, out var row2);
            ...
            {
                var first = code[Offset + 1]; //1st index
                var sec = code[Offset + 2];   //2nd
                var third = code[Offset + 3]; //3rd
                var forth = code[Offset + 4]; //string type
                var num = new byte[] { first, sec, third, 0 };
    ...
    {
        tableIndex = (index & MemberRefParrent) switch
        {
            MemberRefParrent_TYPEDEF => MemberRefParentType.TypeDef,
            MemberRefParrent_TYPEREF => MemberRefParentType.TypeRef,
            MemberRefParrent_MODULEREF => MemberRefParentType.ModuleRef,
            MemberRefParrent_METHODDEF => MemberRefParentType.MethodDef,
            MemberRefParrent_TYPESPEC => MemberRefParentType.TypeSpec,
            _ => 0
        };
        row = index >> 3;
    }
